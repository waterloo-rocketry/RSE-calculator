...
def test_calculate_fuel_mass_values():
    ...
    test_set1 = ECG.calculate_fuel_mass_values(range(5), cm.engine_info, end_of_burn = 4)
    correct_set1 = [6.929129, 5.86934675, 4.8095645, 3.74978225, 2.69000]

    test_set2 = ECG.calculate_fuel_mass_values(range(5), cm.engine_info, end_of_burn = 3)
...
def test_with_sample_file():
    ...
    for idx in range(len(raw_dat.time_s)): # all lists the same length
        ...
        assert abs(nlc.liquid_mass_lb[idx] - float(current_line_split[1])) < ERROR_TOLERANCE
        assert abs(nlc.liquid_cg_in[idx] - float(current_line_split[2])) < ERROR_TOLERANCE
        assert abs(nvc.vapour_mass_lb[idx] - float(current_line_split[3])) < ERROR_TOLERANCE
        assert abs(nvc.vapour_cg_in[idx] - float(current_line_split[4])) < ERROR_TOLERANCE
        assert abs(raw_dat.adjusted_mass_lb[idx] - float(current_line_split[5])) < ERROR_TOLERANCE
        assert abs(ecg.NOS_CG_in[idx] - float(current_line_split[6])) < ERROR_TOLERANCE
        assert abs(ecg.fuel_mass_lb[idx] - float(current_line_split[7])) < ERROR_TOLERANCE
        assert ecg.fuel_CG_in == float(current_line_split[8])
        assert abs(ecg.propellant_mass_lb[idx] - float(current_line_split[9])) < ERROR_TOLERANCE
        assert abs(ecg.propellant_CG_in[idx] - float(current_line_split[10])) < ERROR_TOLERANCE
