...
def test_with_sample_file():
    ...
    for idx in range(len(raw_dat.time_s)): # all lists the same length
        ...
        assert abs(raw_dat.adjusted_mass_lb[idx] - float(current_line_split[1])) < ERROR_TOLERANCE
        assert abs(nmv.NOS_mass_kg[idx] - float(current_line_split[2])) < ERROR_TOLERANCE
        ...
        assert abs(nmv.liquid_volume_m3[idx] - float(current_line_split[5])) < ERROR_TOLERANCE
        assert abs(nmv.vapour_volume_m3[idx] - float(current_line_split[6])) < ERROR_TOLERANCE
        assert abs(nmv.liquid_mass_kg[idx] - \
            float(current_line_split[7])) < ERROR_TOLERANCE
        assert abs(nmv.vapour_mass_kg[idx] - float(current_line_split[8])) < ERROR_TOLERANCE
