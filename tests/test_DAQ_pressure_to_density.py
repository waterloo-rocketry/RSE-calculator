...
def test_with_sample_file():
    ...
    for idx in range(len(raw_dat.time_s)):  # all lists the same length
        ...
        assert abs(
            dpd_object.index_with_closest_p[idx] - float(current_line_split[2])) < 0.001
        assert abs(dpd_object.closest_p[idx] -
                   float(current_line_split[3])) < 0.001
        assert abs(dpd_object.next_p[idx] -
                   float(current_line_split[4])) < 0.001
        assert abs(dpd_object.t_reduced[idx] -
                   float(current_line_split[5])) < 0.001
        assert abs(
            dpd_object.one_minus_t_reduced[idx] - float(current_line_split[6])) < 0.001
        assert abs(dpd_object.reciprocal_t_reduced_minus_one[idx] -
                   float(current_line_split[7])) < 0.001
        assert abs(
            dpd_object.density_liquid_kg_m3[idx] - float(current_line_split[8])) < 0.001
        assert abs(
            dpd_object.gas_density_kg_m3[idx] - float(current_line_split[9])) < 0.001
