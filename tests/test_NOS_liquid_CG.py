...
def test_with_sample_file():
    ...
    for idx in range(len(raw_dat.time_s)): # all lists the same length
        ...
        assert abs(nmv.liquid_volume_m3[idx] - float(current_line_split[1])) < ERROR_TOLERANCE
        assert nlc.case[idx] == float(current_line_split[2])
        assert abs(nlc.liquid_height_m[idx] - float(current_line_split[3])) < ERROR_TOLERANCE
        assert abs(nmv.DAQ_pressure_to_density_data.density_liquid_kg_m3[idx] - \
                float(current_line_split[4])) < ERROR_TOLERANCE
        assert abs(nmv.liquid_mass_kg[idx] - float(current_line_split[5])) < ERROR_TOLERANCE
        assert abs(nlc.liquid_cg_m[idx] - float(current_line_split[6])) < ERROR_TOLERANCE
        assert abs(nlc.liquid_mass_lb[idx] - float(current_line_split[7])) < ERROR_TOLERANCE
        assert abs(nlc.liquid_cg_in[idx] - float(current_line_split[8])) < ERROR_TOLERANCE
